...
import com.inception.android.services.SeenMessageStore
import com.inception.android.ui.ChatState
import com.inception.android.ui.PrivateChatManager
import com.inception.android.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.inception.android.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (!content.startsWith("inception1:")) return@launch

                val base64Content = content.removePrefix("inception1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = InceptionPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.inception.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.inception.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = InceptionFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.inception.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = InceptionMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        com.inception.android.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
