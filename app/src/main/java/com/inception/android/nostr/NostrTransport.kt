...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.inception.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    com.inception.android.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedInception.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedInception.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                val embedded = NostrEmbeddedInception.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = (try {
                com.inception.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }) as? com.inception.android.geohash.ChannelID.Location
            if (selected == null) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded Inception packet without recipient peer ID
                val embedded = NostrEmbeddedInception.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.inception.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            if (ContactIdentityResolver.isNoiseKeyHex(peerID)) {
                val noiseKey = ContactIdentityResolver.bytesFromHex(peerID) ?: return null
                val favoriteStatus = com.inception.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (ContactIdentityResolver.isMeshPeerId(peerID)) {
                val fallbackStatus = com.inception.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
