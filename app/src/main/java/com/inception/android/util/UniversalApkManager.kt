...
import com.inception.android.BuildConfig
import com.inception.android.net.ArtiTorManager
import com.inception.android.net.OkHttpProvider
...
class UniversalApkManager(
    ...
    companion object {
        ...
        private const val APK_FILE_PREFIX = "inception-universal-"
    ...
    private suspend fun executeDownloadRequest(
        ...
        val request = Request.Builder()
            ...
            .url(endpointUrl)
            .addHeader("User-Agent", "Inception-Android")
            .apply {
                if (existingBytes > 0L) {
    ...
    private fun validateDownloadedApk(tempFile: File, source: ApkDownloadSource) {
        if (!verifyApkSignature(tempFile)) {
            clearPartialDownload()
            throw ApkDownloadException(
                message = "APK from ${source.id} is not signed by a trusted Inception release key.",
    ...
    private fun downloadedVersionName(apkFile: File): String {
        ...
        if (packageInfo.packageName != context.packageName) {
            invalidDownloadedApk(ApkDownloadFailureReason.NotInception, "wrong package")
        }
