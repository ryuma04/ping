...
class UnifiedMeshService(
    ...
    override fun sendFavoriteNotification(peerID: String, isFavorite: Boolean) {
        val myNpub = try {
            com.inception.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub
        } catch (_: Exception) {
            null
        }
    ...
    private fun mergedPeerIDs(): List<String> {
        ...
        try { ids.addAll(com.inception.android.services.AppStateStore.peers.value) } catch (_: Exception) { }
