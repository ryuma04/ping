...
import com.inception.android.ui.debug.DebugSettingsManager
import com.inception.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    private fun isClientRoleEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.inception.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true })
    }
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        val dbg = try { com.inception.android.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        Log.d(TAG, "Connecting to inception device: $deviceAddress (peerID: $peerID)")
        ...
        val gattCallback = object : BluetoothGattCallback() {
            ...
            override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
                ...
                val packet = InceptionPacket.fromBinaryData(value)
                if (packet != null) {
