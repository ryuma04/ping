...
class MeshCore(
    ...
    data class Hooks(
         ...
         */
        val onMessageReceived: ((InceptionMessage) -> Boolean)? = null,
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.inception.android.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerInfoFromVerifiedAnnouncement(
                ...
                isVerified: Boolean,
                capabilities: com.inception.android.model.PeerCapabilities?
            ): Boolean {
            ...
            override fun onDeliveryAckReceived(messageID: String, peerID: String) {
                try {
                    com.inception.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.inception.android.model.DeliveryStatus.Delivered(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
            ...
            override fun onReadReceiptReceived(messageID: String, peerID: String) {
                try {
                    com.inception.android.services.AppStateStore.updatePrivateMessageStatus(
                        messageID,
                        com.inception.android.model.DeliveryStatus.Read(peerID, java.util.Date())
                    )
                } catch (_: Exception) { }
    ...
    fun sendFileBroadcast(file: InceptionFilePacket) {
        try {
    ...
    fun sendFilePrivate(recipientPeerID: String, file: InceptionFilePacket) {
        val payload = file.encode() ?: return
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.inception.android.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.inception.android.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.inception.android.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.inception.android.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.inception.android.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
