...
/**
 * Shared transport send wrapper that applies inception packet fragmentation and
...
class FragmentingPacketSender(
    ...
    private fun packetsForTransport(routed: RoutedPacket): List<InceptionPacket>? {
        routed.preparedPackets?.let { prepared ->
            if (prepared.isEmpty() ||
                prepared.size > com.inception.android.util.AppConstants.Fragmentation.MAX_FRAGMENTS_PER_ID) {
