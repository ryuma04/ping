...
import com.inception.android.model.FragmentPayload
...
class FragmentManager {
    ...
    companion object {
        ...
        private const val FRAGMENT_SIZE_THRESHOLD = com.inception.android.util.AppConstants.Fragmentation.FRAGMENT_SIZE_THRESHOLD // Matches iOS: if data.count > 512
        private const val MAX_FRAGMENT_SIZE = com.inception.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_SIZE        // Matches iOS: maxFragmentSize = 469 
        private const val FRAGMENT_TIMEOUT = com.inception.android.util.AppConstants.Fragmentation.FRAGMENT_TIMEOUT_MS     // Matches iOS: 30 seconds cleanup
        private const val CLEANUP_INTERVAL = com.inception.android.util.AppConstants.Fragmentation.CLEANUP_INTERVAL_MS     // 10 seconds cleanup check
    }
    ...
    fun createFragments(packet: InceptionPacket): List<InceptionPacket> =
     ...
     * UInt16 wire limit.
     */
    fun createFragments(packet: InceptionPacket, maxFragments: Int): List<InceptionPacket> {
        try {
            ...
            val fragments = mutableListOf<InceptionPacket>()
            ...
            for (index in fragmentChunks.indices) {
                ...
                val fragmentPacket = InceptionPacket(
     ...
     */
    fun handleFragment(packet: InceptionPacket): InceptionPacket? {
        ...
        try {
            ...
            val maxFragments = com.inception.android.util.AppConstants.Fragmentation.MAX_FRAGMENTS_PER_ID
            ...
            synchronized(fragmentStateLock) {
                ...
                if (isNewSet) {
                    val maxActive = com.inception.android.util.AppConstants.Fragmentation.MAX_ACTIVE_FRAGMENT_SETS
                ...
                val maxTotalBytes = com.inception.android.util.AppConstants.Fragmentation.MAX_FRAGMENT_TOTAL_BYTES
                ...
                val maxGlobalBytes = com.inception.android.util.AppConstants.Fragmentation.MAX_GLOBAL_FRAGMENT_TOTAL_BYTES
                ...
                if (fragmentMap.size == expectedTotal) {
                    ...
                    val originalPacket = InceptionPacket.fromBinaryData(reassembledData.toByteArray())
...
interface FragmentManagerDelegate {
    fun onPacketReassembled(packet: InceptionPacket)
}
