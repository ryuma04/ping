...
class BluetoothConnectionManager(
    ...
    private fun isGattServerEnabled(): Boolean {
        return isBleTransportEnabled() &&
            (try { com.inception.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true })
    }
    ...
    init {
        ...
        try {
            val dbg = com.inception.android.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun sendPacketToLink(deviceAddress: String, linkID: String, packet: InceptionPacket): Boolean {
        if (!isActive || !isBleTransportEnabled()) return false
