...
import com.inception.android.nostr.NostrTransport
import com.inception.android.util.AppConstants
...
class MessageRouter private constructor(
            ...
            val instance = INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                        ...
                        try {
                            com.inception.android.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                        } catch (_: Exception) {}
                        INSTANCE = instance
                    }
                }
    ...
    private val favoriteListener = object: com.inception.android.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String): RouteResult {
        ...
        if (com.inception.android.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}… id=${messageID.take(8)}…")
            val recipientHex = com.inception.android.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                val sourceGeohash = com.inception.android.nostr.GeohashConversationRegistry.get(toPeerID)
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (com.inception.android.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = com.inception.android.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { com.inception.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
        val resolution = ContactDirectory.resolve(toPeerID)
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        ...
        val meshTarget = resolution.meshPeerID ?: toPeerID.takeIf { ContactIdentityResolver.isMeshPeerId(it) }
        if (meshTarget != null && mesh.getPeerInfo(meshTarget)?.isConnected == true && mesh.hasEstablishedSession(meshTarget)) {
            val myNpub = try { com.inception.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (ContactIdentityResolver.isNoiseKeyHex(target)) {
                ...
                val fav = com.inception.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else if (ContactIdentityResolver.isMeshPeerId(target)) {
                val fav = com.inception.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(target)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
