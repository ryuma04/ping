...
class MessageRetentionService private constructor(private val context: Context) {
   ...
   suspend fun saveMessage(message: InceptionMessage, forChannel: String) = withContext(Dispatchers.IO) {
   ...
   suspend fun loadMessagesForChannel(channel: String): List<InceptionMessage> = withContext(Dispatchers.IO) {
   ...
   private fun loadMessagesFromFile(file: File): List<InceptionMessage> {
       if (!file.exists()) {
           return emptyList()
       }
       ...
       return try {
           FileInputStream(file).use { fis ->
               ObjectInputStream(fis).use { ois ->
                   ...
                   ois.readObject() as List<InceptionMessage>
               }
           }
       } catch (e: Exception) {
   ...
   private fun saveMessagesToFile(file: File, messages: List<InceptionMessage>) {
