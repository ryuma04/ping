package com.inception.android.protocol
...
/**
 * Message types with Noise Protocol and Mesh support
 */
 ...
 * DEFLATE output is not canonical and clients use different encoders, so
...
data class InceptionPacket(
    ...
    var ttl: UByte,
    var route: List<ByteArray>? = null, // Optional source route: ordered list of peerIDs (8 bytes each)
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = InceptionPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.inception.android.util.AppConstants.SYNC_TTL_HOPS, // Use fixed TTL=0 for signing
    ...
    companion object {
        fun fromBinaryData(data: ByteArray): InceptionPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: InceptionPacket, padding: Boolean = true): ByteArray? {
        try {
            val maxPayloadLength = com.inception.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
    ...
    fun decode(data: ByteArray): InceptionPacket? =
    ...
    ): InceptionPacket? = decode(data, decompress)
    ...
    private fun decodeCore(
        ...
        try {
            ...
            if (payloadLength > com.inception.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length $payloadLength exceeds maximum allowed (${com.inception.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
            ...
            val payload = if (isCompressed) {
                ...
                val maxExpandedSize = com.inception.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH
