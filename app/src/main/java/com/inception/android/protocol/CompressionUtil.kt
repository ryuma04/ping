...
/**
 * Compression utilities - 100% zlib / raw deflate implementation.
 * Uses the same zlib algorithm as iOS compression for cross-platform compatibility.
 */
object CompressionUtil {
    private const val COMPRESSION_THRESHOLD = com.inception.android.util.AppConstants.Protocol.COMPRESSION_THRESHOLD_BYTES
     ...
     * Uses raw deflate format (no headers) for interoperability
     */
    fun compress(data: ByteArray): ByteArray? {
        ...
        try {
            ...
            return if (compressedData.isNotEmpty() && compressedData.size < data.size) {
                compressedData
            } else {
                null
            }
