...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.inception.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.inception.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: InceptionMessage) {
        ...
        try { com.inception.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: InceptionMessage) {
        ...
        try { com.inception.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessage(peerID: String, message: InceptionMessage) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.inception.android.services.AppStateStore.addPrivateMessage(
                conversationID,
                message
            )
        } catch (_: Exception) {
    ...
    suspend fun addPrivateMessageDurably(
        ...
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.inception.android.services.AppStateStore.addPrivateMessageDurably(
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: InceptionMessage) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        val accepted = try {
            com.inception.android.services.AppStateStore.addPrivateMessage(
                peerID = conversationID,
    ...
    fun clearPrivateMessages(peerID: String) {
        val conversationID = ContactDirectory.canonicalConversationId(peerID)
        com.inception.android.services.AppStateStore.deletePrivateConversation(conversationID)
     ...
     */
    fun generateMessageKey(message: InceptionMessage): String {
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.inception.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
    ...
    fun removeMessageById(messageID: String) {
        ...
        run {
            ...
            if (changed) {
                ...
                com.inception.android.services.AppStateStore.removePrivateMessage(messageID)
            }
        }
        // Channels
        run {
