...
import com.inception.android.services.PrivateMessageArrivalOrder
...
internal data class ConversationSummary(
    ...
    val latestMessageType: InceptionMessageType,
...
internal fun buildConversationSummaries(
    ...
    privateChats: Map<String, List<InceptionMessage>>,
    currentUserIdentifiers: Set<String>,
    canonicalize: (String) -> String,
    isMessageRead: (InceptionMessage) -> Boolean,
    ...
    val messagesByCanonicalID = linkedMapOf<String, MutableList<InceptionMessage>>()
    ...
    return messagesByCanonicalID.mapNotNull { (normalizedConversationID, sourceMessages) ->
        ...
        fun activityOrder(message: InceptionMessage): Long =
            PrivateMessageArrivalOrder.sequenceOf(message.id) ?: message.timestamp.time

        val latest = messages.maxWithOrNull(
            compareBy<InceptionMessage>(::activityOrder).thenBy { it.id }
        ) ?: return@mapNotNull null
        fun isOutgoing(message: InceptionMessage): Boolean =
...
private fun InceptionMessage.conversationPreview(): String {
    val preview = when (type) {
        InceptionMessageType.File -> content
            .substringAfterLast('/')
            .substringAfterLast('\\')
        else -> content
    }
