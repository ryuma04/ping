...
import com.inception.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { com.inception.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }

        // Clean up channel members who disconnected
        channelManager.cleanupDisconnectedMembers(mergedPeers, getMyPeerID())

        runCatching { com.inception.android.services.AppStateStore.canonicalizePrivateChats() }
        ...
        state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
            ...
            if (canonical != currentPeer) {
                com.inception.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                    state = state,
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.inception.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    private fun checkAndTriggerMeshMentionNotification(message: InceptionMessage) {
        try {
     ...
     */
    private fun sendReadReceiptIfFocused(message: InceptionMessage) {
        ...
        if (shouldSendReadReceipt) {
            ...
            try {
                val meshPeerID = ContactDirectory.resolve(senderConversationID).meshPeerID
                    ?: senderPeerID.takeIf {
                        com.inception.android.services.ContactIdentityResolver.isMeshPeerId(it)
                    }
                if (meshPeerID != null &&
     ...
     */
    fun getPeerInfo(peerID: String): com.inception.android.mesh.PeerInfo? {
