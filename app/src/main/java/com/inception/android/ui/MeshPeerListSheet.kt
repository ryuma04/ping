...
import com.inception.android.core.ui.component.sheet.InceptionSheetCenterTopBar
...
import com.inception.android.favorites.FavoriteRelationship
...
fun MeshPeerListSheet(
    ...
    val wifiAwareConnected by com.inception.android.wifiaware.WifiAwareController.connectedPeers.collectAsStateWithLifecycle()
        ...
        InceptionBottomSheet(
            ...
            Box(modifier = Modifier.fillMaxWidth()) {
                LazyColumn(
                    ...
                    when {
                        conversationStoreState is
                            com.inception.android.services.ConversationStoreState.Loading &&
                        ...
                        conversationStoreState is
                            com.inception.android.services.ConversationStoreState.Error &&
                            conversations.isEmpty() -> {
                ...
                InceptionSheetTopBar(
                    title = {
                        InceptionSheetTitle(text = stringResource(id = R.string.your_network))
                    },
...
private fun ConversationRow(
    ...
    val basePreview = when {
        ...
        conversation.latestMessageType == InceptionMessageType.Image ->
            stringResource(R.string.notification_sent_image)
        conversation.latestMessageType == InceptionMessageType.Audio ->
            stringResource(R.string.notification_sent_voice)
        conversation.latestMessageType == InceptionMessageType.File ->
...
private fun UnreadBadge(
    ...
    AnimatedVisibility(
        visible = count > 0,
        enter = fadeIn(tween(InceptionMotion.STANDARD_MS)) +
            scaleIn(
                initialScale = 0.5f,
                animationSpec = tween(InceptionMotion.STANDARD_MS, easing = FastOutSlowInEasing)
            ),
        exit = fadeOut(tween(InceptionMotion.QUICK_MS)) +
            scaleOut(
                targetScale = 0.5f,
