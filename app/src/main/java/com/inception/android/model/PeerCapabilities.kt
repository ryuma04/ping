 ...
 *
 * A minimal little-endian bitfield that always contains at least one byte.
 * Unknown bits in the low 64 bits are kept so a decode/re-encode cycle does not
 * erase capabilities added by newer clients.
 */
@Parcelize
data class PeerCapabilities(val rawValue: Long) : Parcelable {
    ...
    companion object {
        ...
        /** Noise-encrypted private file packet using payload type 0x20. */
        val PRIVATE_MEDIA = PeerCapabilities(1L shl 8)

        val PRIVATE_MEDIA_RECEIPTS = PeerCapabilities(1L shl 9)

        /** Reserved for non-destructive noise replacement. */
        ...
        /**
         * Decode the low 64 bits and ignore any future extension bytes.
         */
