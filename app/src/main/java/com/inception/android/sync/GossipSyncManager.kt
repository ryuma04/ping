package com.inception.android.sync

import android.util.Log
import com.inception.android.mesh.BluetoothPacketBroadcaster
...
class GossipSyncManager(
    ...
    interface Delegate {
        ...
        fun sendPacketToPeer(peerID: String, packet: InceptionPacket)
        fun signPacketForBroadcast(packet: InceptionPacket): InceptionPacket
    }
    ...
    private val messages = LinkedHashMap<String, InceptionPacket>()
    // - announcements: only keep latest per sender peerID
    private val latestAnnouncementByPeer = ConcurrentHashMap<String, Pair<String, InceptionPacket>>()
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.inception.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: InceptionPacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.inception.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.inception.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = InceptionPacket(
            ...
            payload = payload,
            ttl = com.inception.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = InceptionPacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.inception.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.inception.android.util.AppConstants.SYNC_TTL_HOPS)
    ...
    private fun buildGcsPayload(): ByteArray {
        ...
        val list = ArrayList<InceptionPacket>()
        // announcements
