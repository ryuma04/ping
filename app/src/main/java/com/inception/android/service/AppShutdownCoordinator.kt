...
object AppShutdownCoordinator {
    ...
    fun requestFullShutdownAndKill(
        ...
        val job = scope.launch {
            ...
            try {
                val intent = android.content.Intent(com.inception.android.util.AppConstants.UI.ACTION_FORCE_FINISH)
                    .setPackage(app.packageName)
                app.sendBroadcast(intent, com.inception.android.util.AppConstants.UI.PERMISSION_FORCE_FINISH)
            } catch (_: Exception) { }

            // Stop mesh (best-effort)
            try { mesh?.stopServices() } catch (_: Exception) { }
            try { com.inception.android.nostr.NostrRelayManager.shared.disconnect() } catch (_: Exception) { }
            try { com.inception.android.mesh.PowerManager.getInstance(app).shutdown() } catch (_: Exception) { }
            ...
            try { com.inception.android.services.AppStateStore.clear() } catch (_: Exception) { }
