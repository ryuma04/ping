...
object TransportBridgeService {
    ...
    interface TransportLayer {
         ...
         */
        fun sendToPeer(peerID: String, packet: InceptionPacket) { }
    }
     ...
     */
    fun sendToPeer(sourceId: String, peerID: String, packet: InceptionPacket) {
        val targets = transports.filterKeys { it != sourceId }
        if (targets.isEmpty()) return
        val forwardedPacket =
     ...
     */
    fun sendToPeerFromLocal(peerID: String, packet: InceptionPacket) {
        val targets = transports.toMap()
        if (targets.isEmpty()) return
    ...
    private fun prepareForwardedPacket(kind: String, packet: InceptionPacket): PreparedForward? {
    ...
    private fun logicalPacketId(packet: InceptionPacket): String {
